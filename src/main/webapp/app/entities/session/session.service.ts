...
export class SessionService {
  ...
  protected convertDateFromClient(session: ISession): ISession {
    const copy: ISession = Object.assign({}, session, {
      sessionDateTime: session.sessionDateTime && session.sessionDateTime.isValid() ? session.sessionDateTime.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((session: ISession) => {
        session.sessionDateTime = session.sessionDateTime ? moment(session.sessionDateTime) : undefined;
      });
    }
    return res;
  }
