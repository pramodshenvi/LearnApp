...
export class CourseModifySMEDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.course && this.course.courseSMEs) this.courseSMEList = [...this.course.courseSMEs];
  }
  ...
  deleteCourseSME(index: number): void {
    if (this.courseSMEList.length > 0) this.courseSMEList.splice(index, 1);
  }

  onDropdownValueSelection(selection: string): void {
    if (this.course) {
      this.courseSMEList = this.courseSMEList || [];
      if (!this.courseSMEList.includes(selection)) this.courseSMEList.push(selection);
        ...
        this.userService.queryMatchingUsers(courseSME).subscribe(s => {
          ...
          if (matchingUserList && matchingUserList.length > 0) {
            matchingUserList.forEach(e => {
              this.courseSMEDropdownList.push({
                name: (e.firstName ? e.firstName : '') + (e.lastName ? ' ' + e.lastName : '') + '|' + (e.login ? ' ' + e.login : '')
              });
            });
          }
        });
